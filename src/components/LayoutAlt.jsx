...
export function LayoutAlt() {
  ...
  return (<>
    ...
    <ul
      className="w-full container mx-auto grid grid-cols-1 md:grid-cols-3 items-start gap-4">
      {cards.map((card, index) => (
        <motion.div
          layoutId={`card-${card.title}-${id}`}
          key={card.title}
          onClick={() => setActive(card)}
          className={`flex flex-col ${index === 0 || index === 3 ? "col-span-2" : "col-span-1"} hover:bg-neutral-50 dark:hover:bg-neutral-800 rounded-xl cursor-pointer`}>
